...
if [ -e ${CONFIGDIR}/config.ini ]; then
	...
	echo "Replace the default port with ${SERVERPORT} and set web root to ${URLBASE}."
	sed -i s#http_port\ =\ 8090#http_port\ =\ ${SERVERPORT}# ${CONFIGDIR}/config.ini
...
if [ -e /etc/systemd/system/${USER}.service ]; then
	...
	sed -i s#ExecStart=xxxx#ExecStart=/usr/bin/docker\ run\ -v\ ${CONFIGDIR}:${CONFIGDIR}\ -v\ ${DATADIR}:${DATADIR}\ \/etc\/localtime:\/etc\/localtime:ro\ -p\ ${SERVERPORT}:${SERVERPORT}\ --name=${USER}\ ${USER}# ${USER}.service
